...
export function Header() {
  ...
  return <header className="fixed inset-x-0 top-0 z-50 border-b border-white/8 bg-ink/75 backdrop-blur-xl">
    <div className="container-shell flex h-18 items-center justify-between">
      ...
      <button className="grid size-11 place-items-center rounded-full border border-white/12 md:hidden" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-controls="mobile-menu" aria-label={open ? "Fechar menu" : "Abrir menu"}>{open ? <HiX size={24} /> : <HiOutlineMenuAlt4 size={24} />}</button>
    </div>
    {open && <div id="mobile-menu" className="border-t border-white/8 bg-ink md:hidden">
      <nav className="container-shell flex flex-col gap-1 py-5" aria-label="Navegação mobile">
        {links.map((link) => <Link key={link.href} href={link.href} onClick={() => setOpen(false)} className="rounded-xl px-4 py-3 text-lg font-bold hover:bg-white/5 hover:text-mint">{link.label}</Link>)}
      </nav>
    </div>}
  </header>;
}

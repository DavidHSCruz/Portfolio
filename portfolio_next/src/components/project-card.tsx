...
export function ProjectCard({ project, priority = false }: { project: Project; priority?: boolean }) {
  return <Link href={`/projetos/${project.slug}`} aria-label={`Ver detalhes de ${project.name}`} className={`group relative flex min-h-[430px] overflow-hidden rounded-3xl border border-white/10 bg-panel ${priority ? "md:col-span-2 md:min-h-[520px]" : ""}`}>

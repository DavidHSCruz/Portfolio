...
export async function ProjectsShowcase({ limit, heading = true }: { limit?: number; heading?: boolean }) {
  ...
  return <section className="container-shell py-24 md:py-36">
    {heading && <div className="flex flex-col justify-between gap-8 md:flex-row md:items-end"><SectionHeading eyebrow="Trabalho selecionado" title="Projetos que saíram da ideia." body="A vitrine é atualizada a partir dos projetos que recebem informações completas no GitHub." />{limit && <Link href="/projetos" className="shrink-0 font-bold text-mint hover:underline">Ver todos os projetos →</Link>}</div>}
